...
class CollateFunc(object):
    ...
    def __init__(
        ...
        self.sentence_length = sentence_length * 100 # in frames
    ...
    def __call__(self, batch):
        ...
        if self.sentence_length > 0:
            random_select = []
            for x in xs:
                if x.shape[0] - 1 <=  self.sentence_length:
                    start = 0
                else:
                    start = random.randint(0, x.shape[0] - self.sentence_length - 1)
                stop = start + self.sentence_length
                random_select.append(x[start:stop, :])
            xs = random_select
